...
const focusColors: Record<SuggestionFocus, string> = {
    ...
    pengurangan_fitur: 'bg-orange-100 text-orange-700 dark:bg-orange-950 dark:text-orange-400',
    tampilan: 'bg-purple-100 text-purple-700 dark:bg-purple-950 dark:text-purple-400',
    performa: 'bg-green-100 text-green-700 dark:bg-green-950 dark:text-green-400',
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex flex-col gap-6 p-4 md:p-6">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 class="text-2xl font-bold tracking-tight">Saran & Masukan</h1>
                    <p class="text-sm text-muted-foreground">Berikan saran untuk pengembangan aplikasi.</p>
                </div>
                <Button as-child>
            ...
            <Card>
                <CardContent class="pt-6">
                    <div class="grid gap-3 sm:grid-cols-3">
                        <div>
                            <label class="mb-1.5 block text-xs font-medium text-muted-foreground">Fokus</label>
                            <Select v-model="focusFilter">
                                <SelectTrigger><SelectValue placeholder="Semua fokus" /></SelectTrigger>
                                <SelectContent>
                                    <SelectItem value="">Semua</SelectItem>
                                    <SelectItem value="fitur_baru">Fitur Baru</SelectItem>
                                    <SelectItem value="pengurangan_fitur">Pengurangan Fitur</SelectItem>
                                    <SelectItem value="tampilan">Perubahan Tampilan</SelectItem>
                                    <SelectItem value="performa">Performa</SelectItem>
            ...
            <!-- Suggestions Grid -->
            <div v-if="props.suggestions.data.length === 0" class="flex h-64 items-center justify-center">
                <div class="text-center">
                    <Lightbulb class="mx-auto mb-3 size-12 text-muted-foreground/30" />
                    <p class="text-muted-foreground">Belum ada saran. Jadilah yang pertama!</p>
                </div>
            </div>
            ...
            <div v-else class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                ...
                >
                    ...
                    <CardContent class="flex flex-1 flex-col gap-3 pt-4" :class="{ 'rounded-t-lg': !item.image }">
                        <!-- Header -->
                        <div class="flex items-start justify-between gap-2">
                            <h3 class="font-semibold leading-tight">{{ item.title }}</h3>
                            <div class="flex shrink-0 gap-1">
                                <Badge :class="focusColors[item.focus]" class="text-[10px]">
                        ...
                        <div class="flex items-center justify-between border-t pt-3">
                            <div class="flex items-center gap-2">
                                <div class="flex size-6 items-center justify-center rounded-full bg-primary text-[10px] font-bold text-primary-foreground">
                                    {{ item.user?.name?.charAt(0)?.toUpperCase() || '?' }}
                                </div>
                                <div class="text-xs">
                                    <p class="font-medium">{{ item.user?.name || 'Unknown' }}</p>
                                    <p class="text-muted-foreground">{{ formatDate(item.created_at) }}</p>
                                </div>
                            </div>
                            <div class="flex items-center gap-2">
                                <Badge :class="statusColors[item.status]" class="text-[10px]">
            ...
            <!-- Pagination -->
            <div v-if="props.suggestions.last_page > 1" class="flex items-center justify-center gap-1">
                <template v-for="link in props.suggestions.links" :key="link.label">
                    <Button
                        v-if="link.url"
                        size="sm"
        ...
        <Dialog v-model:open="showDeleteDialog">
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Apakah Anda yakin ingin menghapus saran "{{ deleteTarget?.title }}"?
                    </DialogDescription>
                </DialogHeader>
                <DialogFooter>
